...
class SqlQueryService(DbService):
    ...
    def get_customer_query_based_on_rules(self, rules_query):
        if rules_query.get('rules', None):
            joined_query_obj, filter_expression = self.get_joined_query_and_filter(rules_query)
    ...
    def get_joined_query_and_filter(self, query_rules):
        models_map, class_relations = SqlQueryService.get_db_model_relations()
        model_relations = SqlQueryService.get_model_relations_from_rule(query_rules.get('rules', {}))
    ...
    def get_model_relations_from_rule(rules):
        ...
        for a_rule in rules.get('rules', []):
            if 'condition' in a_rule:
                model_relations = model_relations.union(SqlQueryService.get_model_relations_from_rule(a_rule))
            else:
    ...
    def extract_data(results, query_rules):
        ...
        models_map, _ = SqlQueryService.get_db_model_relations()
        model_relations = SqlQueryService.get_model_relations_from_rule(query_rules.get('rules', {}))
